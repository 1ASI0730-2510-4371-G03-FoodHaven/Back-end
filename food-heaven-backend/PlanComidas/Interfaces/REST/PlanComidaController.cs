    ...
    IPlanComidaCommandService commandService) : ControllerBase
{
    ...
    {
        var result = await queryService.Handle(new GetPlanComidaByIdQuery(id));
        return result != null
            ? Ok(PlanComidaResourceFromEntityAssembler.ToResourceFromEntity(result))
            : NotFound($"No se encontró el plan con ID {id}.");
    }
